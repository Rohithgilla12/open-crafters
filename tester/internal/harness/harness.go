...
// Run executes stages up to and including targetSlug (or all stages when
// targetSlug is empty), stopping at the first failure.
func Run(ch Challenge, targetSlug string, programPath string, logf func(string, ...any)) error {
	stages := ch.Stages
	if targetSlug != "" {
		idx := -1
		...
		if idx == -1 {
			return fmt.Errorf("unknown stage %q in challenge %q", targetSlug, ch.Slug)
		}
		stages = stages[:idx+1]
	}
	...
	for i, stage := range stages {
		logf("")
		logf("\x1b[1m[stage %d/%d] %s — %s\x1b[0m", i+1, len(stages), stage.Slug, stage.Name)
		start := time.Now()
		if err := runStage(stage, programPath, logf); err != nil {
